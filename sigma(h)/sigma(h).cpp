// sigma(h).cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main()
{
    ...
    for (double h = 0.001;h <1.001;h +=0.001)
    {
...
#pragma omp parallel for reduction(+:sum1,sum2,sum3)
            ...
            P2 = (double)sum2 / N;
            P3 = (double)sum3 / N;
            fout << h <<" "<<13.53*(P1+P2) <<" "<< 13.53*P3<< endl;
    }
    fout.close();
    return 0;
}
